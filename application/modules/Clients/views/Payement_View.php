        ...
        <div class="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <audio id="fail" src="<?= base_url() ?>/upload/sonnerie/fail.mp3" preload="auto"></audio>
                        ...
                        <div class="card">
                            <?php if (!$groupe): ?>
                             <h1 class="text-center mt-5">Aucun Groupe de paeiment vous est affecté</h1>
                            <?php else: ?>
                            <div class="card-body">
                                <?php include 'include/breadcrumb.php' ?> 
                                <h4 class="card-title text-center m-b-0">Groupes affectés</h4>
                            </div>
...
<script>
    ...
    function sendPay(){
         $('#sendPay').click(function(){
            var mod = $('#modPay').val();
            var montant = $('#montantPay').val();
            var devise = $('#DevPay').val();
            var description = $('#descriptionPay').val();
            var mdp = $('#mdppay').val();
           
            loading()
            if (mod==""||montant==""||devise==""||description==""||mdp=="") {
                displaySuccess('Desolé','Vous devez remplir tous les champs','error');
            }
            else{
                $.ajax({
                    url:"<?= base_url('Clients/pay') ?>",
                    method:"post",
