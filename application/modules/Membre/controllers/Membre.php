...
class Membre extends CI_Controller {
    ...
    public function getOne(){
      ...
      $output['data'] = $one;
      $this->load->view('Membre_Update_View',$output);
    }
...
private function available_amount_when_receive($data){
  ...
  public function param(){
        ...
        $this->form_validation->set_rules('codeGroupe','Code','required|trim');
        $this->form_validation->set_rules('nom','nom','required|trim');
        $this->form_validation->set_rules('Dev','Devise','required|trim');
        $this->form_validation->set_rules('montant','montant','required|trim');
        $this->form_validation->set_rules('description','description','required|trim');
        if($this->form_validation->run()==true){

            $code=generate_code('module','CODE_MODULE','mod');
            $nom=$this->security->xss_clean(html_escape(ucfirst($this->input->post("nom"))));
            $description=$this->security->xss_clean(html_escape($this->input->post("description")));
            $groupe=$this->security->xss_clean(html_escape($this->input->post("codeGroupe")));
            $montant=$this->security->xss_clean(html_escape($this->input->post("montant")));
            $devise=$this->security->xss_clean(html_escape($this->input->post("Dev")));
           if ((int)$montant == 0) {
             echo "Erreur#Veuillez inserer un montant valide et superieur à 0#error";
           }
           else{ 
                $data=array(
                'CODE_MODULE'=>$code,
            ...
            $verification=$this->Model->getOne('module',array('NOM_MODULE'=>$nom,'IS_ACTIF_MODULE'=>9));
            // print_r($verification);
            // exit();
               if ($verification) {
                  echo "Desolé#Ce module existe déjà!!#error";
                
               }else{
                  $this->Model->insert('module',$data);
                  echo "Success#L'enregistrement reussi#success";
               }
    ...
    public function index(){
      ...
      foreach ($devise as $key) {
       $html_dev .= '<option  value="'.$key['CODE_DEVISE'].'">'.$key['NOM_DEVISE'].'('.$key['ABBR_DEVISE'].')</option>';
      }
      $num = 1;
      if ($bd_data) {
